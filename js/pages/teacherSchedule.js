...
function setupExportHandlers(teacherId, context) {
  const exportButtons = document.querySelectorAll('#export-bar-teacher .btn--export');
  ...
  exportButtons.forEach(button => {
    ...
    // Add new listener
    button.addEventListener('click', () => handleExport(button, teacherId, context));
  });
}
...
async function prepareTeacherExportData(teacherId, context) {
  ...
  return scheduleData.schedules.map(schedule => {
    const subject = scheduleData.subjects.find(s => s.id === schedule.subject_id);
    const classInfo = scheduleData.classes.find(c => c.id === schedule.class_id);
    const room = scheduleData.rooms.find(r => r.id === schedule.room_id);
    const timeSlots = generateTimeSlots();
    
    return {
      'วัน': getThaiDayName(schedule.day_of_week),
      'เวลา': timeSlots[schedule.period - 1] || `คาบ ${schedule.period}`,
      'คาบ': schedule.period,
      'วิชา': subject?.subject_name || '',
      'รหัสวิชา': subject?.subject_code || '',
      'ห้องเรียน': classInfo?.class_name || '',
