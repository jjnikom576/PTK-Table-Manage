...
scheduleRoutes.put('/periods/:id', requireJSON, async (c: Context<{ Bindings: Env; Variables: AppVariables }>) => {
  try {
    ...
    if (payload.period_name !== undefined) {
      payload.period_name = payload.period_name.trim();
    ...
    if (payload.period_no !== undefined) {
      const parsed = parseInt(String(payload.period_no), 10);
      if (!Number.isInteger(parsed) || parsed <= 0) {
        return c.json({ success: false, message: 'Period number must be a positive integer' }, 400);
      }
      payload.period_no = parsed;
    }

    if (payload.start_time || payload.end_time) {
      if (!payload.start_time || !payload.end_time) {
        return c.json({ success: false, message: 'Both start and end time are required' }, 400);
      }
      const timeRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
