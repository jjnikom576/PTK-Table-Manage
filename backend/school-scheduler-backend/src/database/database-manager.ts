...
export class DatabaseManager {
  ...
  async updateSubject(subjectId: number, semesterId: number, data: Partial<CreateSubjectRequest>, forYear?: number): Promise<DatabaseResult<Subject[]>> {
    try {
      ...
      // Ensure all remaining rows in the group have consistent payload
      await this.db
  ...
  async deleteSubject(subjectId: number, semesterId: number, forYear?: number): Promise<DatabaseResult> {
    try {
      ...
      if (groupKey) {
        result = await this.db
          .prepare(`DELETE FROM ${tableName} WHERE (group_key = ? OR id = ?) AND semester_id = ?`)
          .bind(groupKey, subjectId, semesterId)
          .run();
      } else {
        result = await this.db
  ...
  private hydrateSubjectRow(subject: any): Subject {
    if (!subject) {
      return subject;
    }
  ...
  private normalizeClassIds(data: CreateSubjectRequest): number[] {
  ...
  private generateSubjectGroupKey(): string {
    try {
